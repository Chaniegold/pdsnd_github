...
def get_filters():
    ...
    print('Hello ' + name + ',')
    print('Nice to meet you!! Let\'s explore some US bikeshare data!')
    ...
    month = input('Which month would you like to see (January, February, March, April, May, June or all)? ').title()
...
def load_data(city, month, day):
    ...
    Args:
        (str) city - name of the city to analyze
        (str) month - the month to filter by, or "all" to apply no month filter
        (str) day - name of the day of week to filter by, or "all" to apply no day filter
    Returns:
...
def trip_duration_stats(df):
    ...
    total_trip_time = df.loc[:, 'Trip Duration'].sum()
    print('The total travel time is: {}'.format(total_trip_time))
          
    # TO DO: display mean travel time
    avg_trip_time = df.loc[:, 'Trip Duration'].mean()
    print('The mean travel time is: {}'.format(avg_trip_time))
...
def raw_input(df):
    ...
    while view_data == 'yes':
        ...
        start_loc += 5
        stop_loc += 5
        view_data = input("Do you wanna view five more rows?  ").lower()
